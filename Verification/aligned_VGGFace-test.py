...
with open("./text/VGGFace2.txt", "r") as f:  # 打开文件
    data = f.read()  # 读取文件
data = data.split('\n')
    ...
    for image_paths in tqdm(data):
        try:
            ...
            for image_path in [path1,path2]:
                ...
                mkdir(os.path.join("./dataset/VGGFace2-test",idy))
                cv2.imwrite(os.path.join("./dataset/VGGFace2-test",idy,image_path.split('/')[-1]),img_)
        except:
            pass