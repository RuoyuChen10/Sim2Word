...
def thresh(args):
    if args.Datasets == "VGGFace2-test":
        ...
        if os.path.exists("./results/text/Celeb-A-results.txt"):
            with open("./results/text/Celeb-A-results.txt", "r") as f:
                datas = f.read().split('\n')
        else:
            raise ValueError("File {} not in path".format("./results/text/Celeb-A-results.txt"))
        score = []
        label = []
        for data in datas:
        ...
        for i in np.linspace(0, 1, 100):
            ...
            score_[score_>=i] = 1
            score_[score_<i] = 0
            acc = sum(np.equal(score_,label))/len(label)
            y.append(acc)
        plt.xlabel("thresh")
        plt.ylabel("ACC")
        plt.plot(x,y)
        plt.savefig('./results/image/Celeb-A.jpg',dpi=400,bbox_inches='tight')
        
def main(args):
    if args.Datasets == "VGGFace2-test":
        if os.path.exists("./results/text/VGGFace2-test-results.txt"):
            with open("./results/text/VGGFace2-test-results.txt", "r") as f:
                datas = f.read().split('\n')
        else:
            raise ValueError("File {} not in path".format("./results/text/VGGFace2-test-results.txt"))
        ...
        if os.path.exists("./results/text/Celeb-A-results2.txt"):
            with open("./results/text/Celeb-A-results2.txt", "r") as f:
                datas = f.read().split('\n')
        else:
            raise ValueError("File {} not in path".format("./results/text/Celeb-A-results2.txt"))
    score = []
    label = []
    for data in datas:
        try:
    ...
    plt.savefig(args.Datasets+'-AUC.jpg',dpi=400,bbox_inches='tight')
        # x = []
        # y = []
