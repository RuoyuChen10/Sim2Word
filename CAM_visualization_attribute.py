...
def main(args):
    ...
    # Read image path
    datas = ['Donald_Trump-alignment.jpg Donald_Trump-alignment.jpg']
    #Read_Datasets(args.Datasets)
    ...
    for data in tqdm(datas):
        ...
        mask1,id_1,scores1 = cam1(torch.tensor([image1.cpu().numpy()], requires_grad=True).cuda(), shape=(224,224))  # cam mask
        mask2,id_2,scores2 = cam1(torch.tensor([image2.cpu().numpy()], requires_grad=True).cuda(), shape=(224,224))  # cam mask
        ...
        # Attribute mask
        mask3,id_3,scores3 = cam2(torch.tensor([image1.cpu().numpy()], requires_grad=True).cuda(), shape=(224,224))  # cam mask
        mask4,id_4,scores4 = cam2(torch.tensor([image2.cpu().numpy()], requires_grad=True).cuda(), shape=(224,224))  # cam mask
        
        mkdir(os.path.join(arrt_cam_save_path,"n"+str(num)+"-1"))
        mkdir(os.path.join(arrt_cam_save_path,"n"+str(num)+"-2"))

        save_attributes_heatmap(path1,id_3,mask3,os.path.join(arrt_cam_save_path,"n"+str(num)+"-1"),args.heatmap_method)
        save_attributes_heatmap(path2,id_4,mask4,os.path.join(arrt_cam_save_path,"n"+str(num)+"-2"),args.heatmap_method)
        num += 1
...
def parse_args():
    ...
    parser.add_argument('--Datasets',
                        type=str,
                        choices=['VGGFace2-train','VGGFace2-test','Celeb-A', 'Defined'],
                        default='Defined',
                        help='Datasets.')
