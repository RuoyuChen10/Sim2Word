...
def main(args):
    ...
    mkdir("./ID_cam_visual_various_channel")
    ...
    for image_path in image_paths:
        ...
        mask, id, scores = cam.get_heatmap_single_channels_out(torch.tensor([image.numpy()], requires_grad=True).cuda())  # cam mask

        save_image_dir_path = "./ID_cam_visual_various_channel/" + image_path.split("/")[-2] + "_" + image_path.split("/")[-1].split(".")[0]
        mkdir(save_image_dir_path)

        for channel_id in range(mask.shape[0]):
            save_path = os.path.join(save_image_dir_path, image_path.split("/")[-2] + "_" + image_path.split("/")[-1].split(".")[0] + "-channel_" + str(channel_id) + ".jpg")

            save_heatmap(image_path, mask[channel_id], save_path)
