...
这个是消融实验，不加属性，只用counterfactual map
"""
...
def main(args):
    ...
    for data in tqdm(datas):
        try:
            ...
            ##### Mask Game #####
            # Image1 attributes
            seg_attr_interpretable1, index1, index2 = seg_attr.ablation_no_attribute(image1,image2)
            seg_attr_interpretable1 = 1 - seg_attr_interpretable1

            # Image2 attributes
            seg_attr_interpretable2, _, __ = seg_attr.ablation_no_attribute(image2,image1)
            ...
            Erasing_image = Erasing(image_1,seg_attr_interpretable1,args.Erasing_method)
            ...
            if args.visualization == True:
                ...
                cv2.imwrite(os.path.join(args.output_dir,"scores-group-"+args.Datasets+"-"+args.verification_net+"-topk-"+str(args.topk)+"-erase-"+args.Erasing_method,"cam",
                                            str(num)+"-n1-n2","mask","attribute.jpg"),
                            seg_attr_interpretable1*255)
                cv2.imwrite(os.path.join(args.output_dir,"scores-group-"+args.Datasets+"-"+args.verification_net+"-topk-"+str(args.topk)+"-erase-"+args.Erasing_method,"cam",
                                            str(num)+"-n1-n2","images","attribute-o.jpg"),
                            Erasing_image)
            ...
            if args.calculate_similarity == True:
                ...
                attr1["image1-f_attr_"] = (torch.cosine_similarity(feature_attr_1[0], feature1[0], dim=0).item(),
                                                torch.cosine_similarity(feature_attr_1[0], feature2[0], dim=0).item())
            # No nan of image 2:
            
            Erasing_image = Erasing(image_2,seg_attr_interpretable2,args.Erasing_method)
            ...
            if args.visualization == True:
                ...
                # Save the Visualization Results
                cv2.imwrite(os.path.join(args.output_dir,"scores-group-"+args.Datasets+"-"+args.verification_net+"-topk-"+str(args.topk)+"-erase-"+args.Erasing_method,"cam",
                                            str(num)+"-n2-n1","mask","attribute.jpg"),
                            seg_attr_interpretable2*255)
                cv2.imwrite(os.path.join(args.output_dir,"scores-group-"+args.Datasets+"-"+args.verification_net+"-topk-"+str(args.topk)+"-erase-"+args.Erasing_method,"cam",
                                            str(num)+"-n2-n1","images","attribute-o.jpg"),
                            Erasing_image)
            ...
            if args.calculate_similarity == True:
                ...
                attr2["image2-f_attr_"] = (torch.cosine_similarity(feature_attr_2[0], feature2[0], dim=0).item(),
            ...
            if feature_attr_1 is not None and feature_attr_2 is not None:
                if args.calculate_similarity == True:
                    attr3["Both_masked-f_attr_"] = torch.cosine_similarity(feature_attr_1[0], feature_attr_2[0], dim=0).item()
